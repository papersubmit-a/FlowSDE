...
class StochasticDiffusion(eqx.Module):
    ...
    def __init__(self, n_blocks, x_size, hidden_size, depth, *, key, model_name):
        ...
        if 'MAF' in model_name:
            flow = MAF(n_blocks, n_size, hidden_size, depth, cond_label_size=cond_size, activation=jnn.tanh,
            ...
            flow = RealNVP(n_blocks, n_size, hidden_size, depth, cond_label_size=cond_size, key=k1, dropout=True,
                           act_norm=True if 'act_norm' in model_name else False, dropout_rate=0.1, model="coupling")
    ...
    def __call__(self, cond, key, args=None, inference=False):
        ...
        if 'one' in self.model_name:
            ...
            if not inference:
                flow, transformed_noise, logJ = self.flow.inverse(dw, cond=_cond, key=dropout_key, inference=inference)
                self = eqx.tree_at(lambda m: m.flow, self, flow)
            else:
            ...
            for i in range(len(self.flow)):
                ...
                if 'dw' in self.model_name:
                    ...
                    if 'diag' in self.model_name:
                        ...
                        _cond = jnp.concatenate([cond, dt[..., None]], axis=-1)
                if not inference:
                    flow, _noise, _logJ = self.flow[i].inverse(dw, cond=_cond, key=dropout_key, inference=inference)
                    flow_list = self.flow.copy()
                    flow_list[i] = flow
                    self = eqx.tree_at(lambda m: m.flow, self, flow_list)
                else:
    ...
    def flow_forward(self, diff, cond, key, dt):
        ...
        if 'diag' in self.model_name:
            ...
            for i in range(len(self.flow)):
                if 'dw' in self.model_name:
                    _cond = jnp.stack([cond[..., i], cond[..., -1]], axis=-1)
                    flow, _zs, _logJ = self.flow[i](diff[..., [i]], cond=_cond, key=key, inference=False)
                    flow_list = self.flow.copy()
                    flow_list[i] = flow
                    self = eqx.tree_at(lambda m: m.flow, self, flow_list)
                else:
            ...
            for i in range(len(self.flow)):
                if 'dw' in self.model_name:
                    flow, _zs, _logJ = self.flow[i](diff[..., [i]], cond=cond, key=key, inference=False)
                    flow_list = self.flow.copy()
                    flow_list[i] = flow
                    self = eqx.tree_at(lambda m: m.flow, self, flow_list)
                else:
            ...
            if 'dw' in self.model_name:
                flow, zs, logJ = self.flow(diff, cond=cond, key=key, inference=False)
                self = eqx.tree_at(lambda m: m.flow, self, flow)
            else:
                _cond = jnp.concatenate([cond, dt[..., None]], axis=-1)
                flow, zs, logJ = self.flow(diff, cond=_cond, key=key, inference=False)
                self = eqx.tree_at(lambda m: m.flow, self, flow)
    ...
    def train_forward(self, diff, cond, key, dts):
        zs, logJ = jax.vmap(self.flow_forward, in_axes=(0, 0, None, 0))(diff, cond, key, dts)
        if 'dw' in self.model_name:
            zs_loss = jnp.sum(0.5 * zs ** 2 / dts + 0.5 * jnp.log(dts), axis=-1)
        else:
            zs_loss = jnp.sum(0.5 * zs ** 2, axis=-1)
        flow_loss = jnp.nanmean(zs_loss) - jnp.nanmean(logJ)
