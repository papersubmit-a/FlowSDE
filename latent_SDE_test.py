...
if __name__ == "__main__":
    ...
    parser.add_argument('--calculation', type=str2bool, default=True)
    ...
    pt_name = f"LatentSDE_{args.data_type}_{data_name_train}_{model_name}_hdr_{args.hidden_size_drift}_ddr_{args.depth_drift}_hdf_{args.hidden_size_diff}_ddf_{args.depth_diff}_p{args.patch_size}"
    job_name = f"LatentSDE_{args.data_type}_{data_name}_{model_name}_hdr_{args.hidden_size_drift}_ddr_{args.depth_drift}_hdf_{args.hidden_size_diff}_ddf_{args.depth_diff}_p{args.patch_size}"
    ...
    if args.eval_and_plot:
        ...
        with open('log_results.txt', 'a') as file:
            file.write(job_name + ':\n')
            file.write(f"rmse: {rmse}, mciw: {mciw}, crps: {crps}" + '\n')
        file.close()
